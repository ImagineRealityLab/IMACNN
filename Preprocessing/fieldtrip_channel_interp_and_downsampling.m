% This script downsamples and interpolates channels as implemented in fieldtrip
...
output_dir='D:\UCL Individual Scene Imagery\EEG_data\fieldtrip preprocessing\';%C:\MATLAB\Individual Scene Imagery\Data\EEG_Data\fieldtrip preprocessing\';
...
% This interpolates missing channels by calculating the average of all
...
% The labels and channels are being sorted correctly
for trial_num=1:size(timelock.trial,1)
%data.trial{1,trial_num}=repmat([1:size(data.trial{1,trial_num},1)]',1,size(data.trial{1,trial_num},2));    
timelock.trial(trial_num,:,:)=timelock.trial(trial_num,double(temp_sorted(:,2)),:);
%data.trial{1,trial_num}=data.trial{1,trial_num}(double(temp_sorted(:,2)),:);% We want to sort the data according to the labels
end
...
% Save the imagery frequency data of this subject at this session
